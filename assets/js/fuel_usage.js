  ...
  try {
    ...
    if (result.success) {
      ...
      if (data && data.length > 0) {
        ...
        if (tableBody)
          tableBody.innerHTML = `<div class="text-center text-muted py-3">No fuel usage records found</div>`;
    ...
    if (loading) loading.style.display = "none";
    if (tableBody)
      tableBody.innerHTML = `<div class="text-center text-danger py-3">Failed to load fuel usages</div>`;
    console.error(err);
...
async function createFuelUsage(payload) {
  try {
    const result = await apiFetch("/fuel-usages", {
      method: "POST",
...
async function updateFuelUsage(id, payload) {
  try {
    const result = await apiFetch(`/fuel-usages/${id}`, {
      method: "PUT",
      body: JSON.stringify(payload),
    });
  ...
  addUsageForm.addEventListener("submit", async (e) => {
    ...
    if (success) {
      bootstrap.Modal.getOrCreateInstance(
        document.getElementById("addUsageModal")
      ).hide();
      addUsageForm.reset();
...
if (editUsageForm) {
  editUsageForm.addEventListener("submit", async (e) => {
    ...
    if (!quantity) return showError("Please enter usage quantity.");
    if (!editSelectedUsageUser)
      return showError("Please select a user or staff.");
    if (!date) return showError("Please select a date.");
