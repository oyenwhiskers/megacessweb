...
async function getAllVehicleBookings({ search = '', bookingFilter = '', page = 1 } = {}) {
    const token = getToken();
    if (!token) return showErrorNoToken("Please login first.");
...
document.getElementById('vehicleBookingTableBody').addEventListener('click', (e) => {
    // Handle Delete
...
document.getElementById("addVehicleBookingForm").addEventListener("submit", async (e) => {
    ...
    if (!payload.vehicle_id || (!payload.user_id && !payload.staff_id)) return showError("Select vehicle and user/staff.");
    ...
    } catch (err) { console.error(err); showError("Error creating booking"); }
...
document.getElementById("updateVehicleBookingForm").addEventListener("submit", async (e) => {
    ...
    } catch (err) { console.error(err); showError("Error updating."); }
